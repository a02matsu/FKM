...
sc1 = 0.96*log(gamma) + 0.47
sc2 = 0.68*log(gamma) + 0.73
scd1 = 1.0 - sc1
scd2 = 1.0 - sc2
# データを取る q の値を設定
# q = [ (1-u)(OMEGA-u) ]^(-s)
...
S3 = collect(scd1+0.3:0.20:scd2-0.3)
S4 = collect(scd2-0.3:0.05:scd2+0.3)
S5 = collect(scd2+0.3:0.2:sc2-0.3)
S6 = collect(sc2-0.3:0.05:sc2+0.3)
S7 = collect(sc2+0.3:0.2:sc1-0.3)
S8 = collect(sc1-0.3:0.05:sc1+0.3)
S9 = collect(sc1+0.3:0.2:Smax)
S = unique(vcat(S1,S2,S3,S4,S5,S6,S7,S8,S9))
Q = qof.(S)
...
##
append!(S,collect(sc2+0.01:0.01:sc2+0.041) )
append!(S,collect(sc2+0.06:0.01:sc2+0.091) )
append!(S,collect(sc2-0.09:0.01:sc2-0.059) )
append!(S,collect(sc2-0.04:0.01:sc2-0.009) )
##
append!(S,collect(dsc1+0.01:0.01:dsc1+0.041) )
append!(S,collect(dsc1+0.06:0.01:dsc1+0.091) )
append!(S,collect(dsc1-0.09:0.01:dsc1-0.059) )
append!(S,collect(dsc1-0.04:0.01:dsc1-0.009) )
##
append!(S,collect(dsc2+0.01:0.01:dsc2+0.041) )
append!(S,collect(dsc2+0.06:0.01:dsc2+0.091) )
append!(S,collect(dsc2-0.09:0.01:dsc2-0.059) )
append!(S,collect(dsc2-0.04:0.01:dsc2-0.009) )
Q = qof.(S)
