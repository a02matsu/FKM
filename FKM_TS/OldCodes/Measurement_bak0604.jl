...
function analysePT(Qmax=16, epsilon=10^(-4), wt=0.05)
  ...
  for a in 1:RANK
    println("Cycle $(a)")
    Sc = [@sprintf("%.4f", val) for val in sof.(Qcs[a])]
    dSc = [@sprintf("%.4f", val) for val in sof.(dQcs[a])]
    Sum = [@sprintf("%.4f", val) for val in sof.(dQcs[a]) + sof.(Qcs[a])] 
    println("\t sc = : [", join(Sc,","),"]" )  
  ...
  for a in 1:RANK
    ...
    println("\t (-) : (",@sprintf("%.4f",p2[a][1]), " +/- ", @sprintf("%.7f",erra2[a]),") log(γ-1/2) + (", @sprintf("%.4f",p2[a][2]), " +/- ", @sprintf("%.7f",errb2[a]), ")" )
  ...
  # GWWとの比較
    ...
    println("\t n=$(poly[a])(-): q'_c^{-$(poly[a])}/Omega^$(poly[a]) = ","(",(@sprintf("%.3f", fac))," +/- ",@sprintf("%.7f",fac_err),")", "(γ-1/2)^(", @sprintf("%.3f",expn), " +/- ", @sprintf("%.7f",expn_err),")")
    #fac = exp(n*(p2[a][2]))
    #expn = n*p2[a][1]
