...
phys = "specificheat"
# 理論値(DT)
zeta(q) = -1 / ((q+1) * (q^2+1) * (2*q^2+q+1) * (2*q^3+q^2-1) * (q-1)^2)
...
for gamma_int in [102400]
  ...
  C1(s) = 1.0
  plot!(1.0:0.05:6.0568, C1, label="analytic", linecolor=:green, linewidth=3.0)
  plot!(6.0568:0.05:10.0, C0, label=nothing, linecolor=:green, linewidth=3.0)

  qd(s) = omega^(-(1-s))
  C0d(s) = g^2 * log(zeta(qd(s)^2))
  C1d(s) = 1.0
  plot!(-5.0568:0.05:0, C1d, label=nothing, linecolor=:green, linewidth=3.0)
  plot!(-9.0568:0.05:-5.0568, C0d, label=nothing, linecolor=:green, linewidth=3.0)
end
