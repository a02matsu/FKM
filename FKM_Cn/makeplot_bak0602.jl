...
# 相転移点を見つける
function findPT(Nc, gamma_int, epsilon=10^(-5))
  # CSVファイルを見つける
...
function plot_gamma(phys, gamma_int)
    ...
    xlabel!("\$s\$")
    ylabel!("$(phys)")
    title!("$(GRAPH), $(phys), \$ \\gamma = $(gamma_int/100) \$")
...
pltq = plot()
for a in 1:RANK
  scatter!(pltq, LG, sof.(Qcs[a],0.0) ,label="")
  scatter!(pltq, LG, sof.(dQcs[a],0.0),label="" )
end
for a in 1:RANK
  F1(x) = model(x,p1[a])
  F2(x) = model(x,p2[a])
...
for a in 1:RANK
  ...
  fac = exp(-n*p1[a][2])
  expn = -n*p1[a][1]
  fac_err =  n * errb1[a] * exp(-n*p1[a][2])
  expn_err = n*erra1[a]
  println("n=$(n)(+): ","(",(@sprintf("%.3f", fac))," +/- ",@sprintf("%.4f",fac_err),")", "(γ-1/2)^(", @sprintf("%.3f",expn), " +/- ", @sprintf("%.4f",expn_err),")")
  ##
  fac = exp(n*p2[a][2])
  expn = n*p2[a][1]
  fac_err =  n * errb2[a] * exp(n*p2[a][2])
  expn_err = n*erra2[a]
  println("n=$(n)(-): ","(",(@sprintf("%.3f", fac))," +/- ",@sprintf("%.7f",fac_err),")", "(γ-1/2)^(", @sprintf("%.3f",expn), " +/- ", @sprintf("%.7f",expn_err),")")
end
