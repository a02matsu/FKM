...
# (q,u)からsへの変換
sof(q,u) = -log(q)/log((1-u)*(OMEGA-u))
# (s,u)からqへの変換
qof(s,u) = ( (1-u)*(OMEGA-u) )^(-s) 
# 近似的な相転移点の位置
qc1 = 0.2518
qc2 = 0.1695
sc1 = sof(qc1,u)
...
Smin = -5.0
Smax = 6.0
S1 = collect(Smin:0.2:scd2-0.1)
S2 = collect(scd2-0.1:0.08:scd1+0.1)
S3 = collect(scd1+0.1:0.30:sc1-0.1)
S4 = collect(sc1-0.1:0.08:sc2+0.1)
S5 = collect(sc2+0.1:0.2:Smax)
S = unique(vcat(S1,S2,S3,S4,S5))
Q = qof.(S,u)
Q = filter(x -> !(qb1-0.02 < x < qb2+0.02), Q)

S = collect(3.1:0.02:3.2)
Q = qof.(S,u)
...
@distributed for (q,i) in block
    ...
    if Ntau > MaxNtau
        ...
        xlabel!("\$\\theta\$")
        title!("TS, \$\\gamma = $(gamma), q=$(q)\$, \$u=0.0\$")
