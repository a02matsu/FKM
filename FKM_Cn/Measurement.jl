...
export calc_obs, findPT, plot_gamma, plot_N, analysePT, plot_gamma_small_q, sof, qof, plot_gamma_large_q, plot_ST
...
function plot_ST(gamma_int)
  ...
  for file in files
      ...
      for (s,e, eerr) in zipped_data
        q = qof(s)
        if q < 1
          push!(y, log( -e / q^minl)  ) # string tension
          push!(err, eerr / abs(e) ) # error 
        else 
          c = (NE-NV) * log( OMEGA^2 * q^2 * (1 - q^2) / (1 - OMEGA^2*q^2) ) + NV * log(OMEGA*q^2)
          println(e ," ", c, " ", OMEGA, " ", q)
          push!(y, log( (OMEGA*q)^(minl) * ( -e + c ) ))
          push!(err, eerr / abs(e) )
        end
      end
      # プロットを追加
